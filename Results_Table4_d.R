...
table = matrix(NA,length(name.vec)*length(dim.vec),14)
...
for (name_n in name.vec) {
  ...
  for (trial_dim_n in dim.vec) {
    ...
    rownames(table) = c("Ex1_d_10","Ex1_d_20","Ex1_d_25","Ex1_d_30",
                        "Ex2_d_10","Ex2_d_20","Ex2_d_25","Ex2_d_30",
                        "Ex3_d_10","Ex3_d_20","Ex3_d_25","Ex3_d_30")
    ii = ii + 1  
  }
}
...
print(table)
write.table(table, file=paste0(work.path,"/Results/Table_d.txt"), row.names=T, col.names=T, sep="|")
