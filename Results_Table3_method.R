...
table = matrix(NA,length(name.vec)*length(method.vec),14)
...
for (name_n in name.vec) {
  ...
  for (method_n in method.vec) {
    ...
    rownames(table) = c("Ex1_EX_Sliced_Calib","Ex1_Rand_Sliced_Calib","Ex1_Full-Calib","Ex1_TR_Sliced_Calib",
                        "Ex2_EX_Sliced_Calib","Ex2_Rand_Sliced_Calib","Ex2_Full-Calib","Ex2_TR_Sliced_Calib",
                        "Ex3_EX_Sliced_Calib","Ex3_Rand_Sliced_Calib","Ex3_Full-Calib","Ex3_TR_Sliced_Calib")
    ii = ii + 1  
  }
}
...
print(table)
write.table(table, file=paste0(work.path,"/Results/Table_method.txt"), row.names=T, col.names=T, sep="|")
